-- SQL Server позволяет получать некоторую информацию о запросах из кэша. Если обновление кэша запросов происходит слишком часто или запросы просто не кэшируются, то в результате этой выборки они отражены не будут

-- Изначально скрипт регистрирует все запросы, которые выполнялись более 1 раза или за последние сутки по количеству чтений+записи/количество выполнений
